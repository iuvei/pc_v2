...
export default class Login extends Component {
    ...
    process(vars) {
        if (vars.points.length < vars.initParticles) for (var i = 0; i < 5; ++i) this.spawnParticle(vars);
        var p, d, t;
        ...
        for (var i = -25; i <= 25; i += 1) {
            for (var j = -25; j <= 25; j += 1) {
                ...
                if (point.d != -1) {
                    ...
                    if (a > 0) {
                        vars.ctx.fillStyle = this.colorString(this.interpolateColors(this.rgbArray(d / 26 - vars.frameNo / 40), [0, 128, 32], .5 + Math.sin(d / 6 - vars.frameNo / 8) / 2));
        ...
        for (var i = -25; i <= 25; i += 1) {
            for (var j = -25; j <= 25; j += 1) {
                ...
                if (point.d != -1) {
                    ...
                    a = 0.15 - Math.pow(d / 50, 4) * 0.15;
                    if (a > 0) {
                        vars.ctx.fillStyle = this.colorString(this.interpolateColors(this.rgbArray( - d / 26 - vars.frameNo / 40), [32, 0, 128], .5 + Math.sin( - d / 6 - vars.frameNo / 8) / 2));
                        vars.ctx.globalAlpha = a;
                        vars.ctx.fillRect(point.x - size / 2, point.y - size / 2, size, size)
                    }
                }
    ...
    frame(vars) {
        ...
        vars.frameNo++;
        this.state.stopAnimation = requestAnimationFrame(()=>{
            this.frame(vars)
        });
        this.process(vars);
        this.draw(vars)
